...
export const ThemeSwitcher = () => {
  ...
  return (
    <div className="theme-switcher-container group relative" ref={dropdownRef}>
      ...
      {isOpen && (
        <div className="theme-switcher-dropdown w-60 border-2 border-theme-border rounded-2xl shadow-2xl backdrop-blur-sm" style={{ backgroundColor: 'white' }}>
        <div className="p-4" style={{ backgroundColor: 'white' }}>
          <div className="text-sm font-bold theme-text mb-4 px-2 text-center">🎨 Choose Your Vibe</div>
          <div className="space-y-2">
            {themes.map((theme) => (
              ...
              >
                <div 
                  className="w-6 h-6 rounded-full border-2 border-theme-border shadow-md"
                  style={{ backgroundColor: theme.colors.primary }}
                />
                <span className="font-semibold">{theme.name}</span>
                {currentTheme.id === theme.id && (
                  <div className="ml-auto w-3 h-3 bg-theme-primary rounded-full animate-pulse shadow-sm" />
                )}
              </button>
            ))}
          </div>
